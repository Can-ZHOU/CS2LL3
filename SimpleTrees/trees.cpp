 ...
 */ 
Tree::~Tree(void) {
  cout << "starting destructor on tree with mother: " << mother.cat << endl;
  for(unsigned int i = 0; i < dtrs.size(); i++) {
    delete dtrs[i];
  }
  cout << "finished destructor on tree with mother: " << mother.cat << endl;
}
...
void Tree::dot_tree(ostream& f) {
  ...
  f << node_count << endl;
  if(mother.terminal) {
    f << node_count << " [color=red,label=\"" << mother.cat << "\"];" << endl;
  }
  else {
    f << node_count << " [color=green,label=\"" << mother.cat << "\"];" << endl;
  }
